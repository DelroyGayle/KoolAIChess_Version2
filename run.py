...
def minimax(chess: Game, wake_game: WakeGame,
    ...
    maxmin_score = (MINUS_INFINITY if is_maximising
                    else INFINITY)
    for from_square, to_square in valid_moves:
        ...
        if (move_result.is_king_in_check or
           ...
           move_result.is_stalemate):
            ...
            if move_result.is_king_in_check:
                # ignore this move since it places
                # the rival's own king in check
                continue
        ...
        if not next_moves:
            ...
            print("MAX/MIN FAILED", next_moves)  # TODO P
            quit()
            return INFINITY if is_maximising else MINUS_INFINITY
        ...
        if is_maximising:
            if the_score > maxmin_score:
                maxmin_score = the_score
                if level == 1:
...
# TODO ADD COMMENTS
def negamax_root(chess: Game, wake_game: WakeGame):
    """ TODO - ADD COMMENTS """

    result = negamax(chess, wake_game,
                     level=0,
                     current_score=0,
                     rival=Rival.COMPUTER,
                     is_maximising=True,
...
def negamax(chess: Game, wake_game: WakeGame,
            level: int,
            current_score: int,
            rival: int,
            is_maximising: bool,
    ...
    for from_square, to_square in moves_list:
        ...
        current_score += (moveinfo.targetvalue +
                          (8 - abs(4 - moveinfo.to_file_number) -
                           abs(4 - moveinfo.to_rank_number)))

        other_rival = (Rival.COMPUTER if rival == Rival.PLAYER
                       else Rival.PLAYER)
        current_score += negamax(chess,
                                 wake_game,
                                 level,
                                 current_score,
                                 other_rival,
        ...
        # restore previous value before continuing the loop
    ...
    # Otherwise return the best determined score
...
def main_part2():
    ...
    while True:
        ...
        Game.evaluation_result = negamax_root(chess, wake_game)
